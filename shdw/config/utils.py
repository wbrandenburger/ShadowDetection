...
def get_config_dirs():
    """Get shdw configuration directories where the configuration
    ...
    if os.environ.get('XDG_CONFIG_DIRS'):
        ...
        dirs += [
            os.path.join(d, 'shdw') for d in
            os.environ.get('XDG_CONFIG_DIRS').split(':')
        ]
    # Take XDG_CONFIG_HOME and ~/.shdw for backwards
    # compatibility
    dirs += [
        os.path.join(get_config_home(), 'shdw'),
        os.path.join(os.path.expanduser('~'), '.shdw')
    ]
    return dirs

def get_config_folder():
    """Get folder where the configuration files are stored,
    e.g. ``~/shdw``. It is XDG compatible, which means that if the
    environment variable ``XDG_CONFIG_HOME`` is defined it will use the
    configuration folder ``XDG_CONFIG_HOME/shdw`` instead.
    ...
    return os.path.join(get_config_home(), "shdw")
